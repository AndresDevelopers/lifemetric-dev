...
- El **rate limiting** sigue prioridad por proveedor: **Cloudflare primero**, **Upstash Redis como fallback** y, si ninguno está configurado o disponible, la request fluye directo hacia Supabase sin una capa adicional de rate limit en la app. El fallback Redis usa comandos nativos (`INCR` + `EXPIRE`) para reducir dependencias críticas en runtime.
